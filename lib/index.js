var express = require('express'), // Required Express Module
    utils = require('./utils'),   // Required Utils Module
    async = require("async");
...
app.get('/I/want/title', async (req, res, next) => {
    try {
        if (!req.query.address) {
            ...
            var addresses = await(utils.getUrls(req.query));

            // Append http:// with url string
            await(utils.appendUrlProtocol(addresses));
            ...
            await(utils.getAllUrlList(addresses, function (err, titleList) {
                if (!err) {
